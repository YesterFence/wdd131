...
let filteredHikes = hikes.filter(function(hike){
    return (
        ...
        hike.tags.find((tag) => tag.toLocaleLowerCase().includes(hikeQuery.toLocaleLowerCase()))
    )
})
...
function ratingTemplate(rating)
{
  let html = `<span
      ...
      for (let i = 1; i <=5; i++)
      {
        if (i <= rating)
        {
          html += `<span aria-hidden='true' class="icon-star">star</span>`
        }
      }
}
